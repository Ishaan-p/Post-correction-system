...
BATCH_SIZE = 8  # Increased batch size for GPU processing
TOTAL_SIZE = 2152
API_KEY = ""
...
def collect_database():
    ...
    for _ in range(TOTAL_SIZE):
        conformer_row = conformer[conformer["id"] == _+4001]
        whisper_row = whisper[whisper["id"] == _+4001]
        wav2vec_row = wav2vec[wav2vec["id"] == _+4001]
        reference_row = reference[reference["id"] == _+4001]
...
def create_prompt(h1, h2, h3):
    ...
    prompt = f'''Perform error correction based on the top3 outputs generated by an Automatic Speech Recognition (ASR) system. The ASR hypotheses are listed in order of their ASR posterior score. Please provide the corrected top1 ASR transcription of the given utterance only.  Do not add any explanations or other words. 
        ...
        Your output: क्या आप बता सकते हैं कि उस इलाके में स्कूल पाँच सौ बीस बच्चों को एक साथ बढ़िया से मैनेज कर पाता है या नहीं
        ...
        Your output: मुझे पता करना है कि उस मेडिकल कॉलेज में शैक्षणिक सत्र दो हज़ार पंद्रह से दो हज़ार बीस के बीच कितना प्रतिष्ठित माना जाता था
        ...
        Your output: मैरे मित्र ने कहा कि उस इलाके में सालाना करीब तीन लाख पचास हजार पर्यटक आते हैं आप यह आंकड़ा देख सकते हैं
...
def correct_batch_sync(batch_data):
...
async def correct_batch(asr_results):
    ...
    for i in range(0, len(asr_results), GPU_BATCH_SIZE):
        ...
        batch_corrections = correct_batch_sync(batch)
...
def create_database_new(batch_data, csv_file="gpt_transcripts_3.csv"):
